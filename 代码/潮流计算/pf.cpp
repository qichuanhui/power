...
auto run = krnl_pf(
    ...
    // ========== 求解并更新Va ==========
    gettimeofday(&start, 0);
    Eigen::VectorXd deltaP_vec(node_num);
    for (int i = 0; i < node_num; ++i) deltaP_vec(i) = deltaP[i];
    deltaP_vec = solver_p->solve(deltaP_vec);  // Eigen 求解
    for (int i = 0; i < node_num; ++i) {
        deltaP[i] = deltaP_vec(i);
        node[i].Va -= deltaP[i];
        // 添加角度规范化
  ...
  // 创建内核执行对象
    ...
    // ========== 求解并更新Vm ==========
    gettimeofday(&start, 0);
    Eigen::VectorXd deltaQ_vec(node_num);
    for (int i = 0; i < node_num; ++i) deltaQ_vec(i) = deltaQ[i];
    deltaQ_vec = solver_pp->solve(deltaQ_vec);  // Eigen 求解
    for (int i = 0; i < node_num; ++i) {
        deltaQ[i] = deltaQ_vec(i);
        node[i].Vm -= deltaQ[i];
    }
    gettimeofday(&finish, 0);
    cost_time = 1000000 * (finish.tv_sec - start.tv_sec) + finish.tv_usec - start.tv_usec;
    ...
    printf("convert                   : %lfs\n", convert_time);
    printf("total                     : %lfs\n", (Ybus_time + fac_time + rhs_time + back_for_time + convert_time));
    printf("------------------------------------------\n");
